...
void AccelStepper::runSpeedWithAcceleration(){
    runSpeed();
    if (_speed == _targetSpeed)
        return;

    uint32_t now = micros();
    if ((now - _lastSpeedTime) >= _speedInterval ) {
        _targetSpeed = constrain(_targetSpeed, -_maxSpeed, _maxSpeed);
        if(abs(_speed - _targetSpeed)<1){
...
void AccelStepper::setSpeedWithAcceleration(float speed, float acceleration){
    ...
    if (acceleration < 0.0)
        acceleration = -acceleration;
    _speedInterval = 1000000.0/acceleration;
